...
// ---------- Single Project Component – optimized with re-triggering animations ----------
const Single = React.memo(({ item, index }) => {
  ...
  // Enhanced animation variants with spring for a creative, smooth feel
  const containerVariants = {
    hidden: { opacity: 0, y: isEven ? -50 : 50 },
    visible: { 
      opacity: 1, 
      y: 0,
      transition: { 
  ...
  const imageVariants = {
    hidden: { opacity: 0, scale: 0.8, x: isEven ? -30 : 30 },
    visible: { 
      opacity: 1, 
      scale: 1, 
      x: 0,
  ...
  const textVariants = {
    hidden: { opacity: 0, y: isEven ? 30 : -30 },
    visible: { 
      opacity: 1, 
      y: 0,
      transition: { 
  ...
  // Memoize gradient colors (unused but kept for consistency)
    ...
    <section className="portfolio-section" style={{ height: '100vh', position: 'relative', backgroundColor: 'black', color: 'white', overflow: 'hidden' }}>
      ...
      <div className="container">
        <motion.div 
          ...
          initial="hidden"
          whileInView="visible"
          viewport={{ amount: 0.3, once: false }}  // 👈 animates every time
        >
...
// ---------- Main Portfolio Component – with sticky progress bar ----------
const Portfolio = () => {
