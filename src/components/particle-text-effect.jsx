...
const DEFAULT_WORDS = ["Welcome To My Portfolio", "I am Gizachew Mohammed", "Full-Stack Developer", "AI & ML Enthusiast", "enjoy exploring"]
...
export function ParticleTextEffect({
  ...
  const wordsDisplayedRef = useRef(0)
  ...
  const nextWord = (word, canvas) => {
    ...
    // Generate new bright color
    const newColor = {
      r: Math.random() * 155 + 100,  // 100-255 (bright red)
      g: Math.random() * 155 + 100,  // 100-255 (bright green)
      b: Math.random() * 155 + 100,  // 100-255 (bright blue)
    }
  ...
  const animate = () => {
    ...
    const isName = currentWord === "I am Gizachew Mohammed"
    const frameLimit = isName ? 280 : 240 
    
    if (frameCountRef.current % frameLimit === 0) {
      wordIndexRef.current = (wordIndexRef.current + 1) % words.length
      wordsDisplayedRef.current++
      
      // Check if we've displayed all words once
      if (wordsDisplayedRef.current >= words.length && !completedRef.current) {
